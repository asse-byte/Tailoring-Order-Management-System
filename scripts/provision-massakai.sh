...
slug="massakai-koutur"
shop_name="Massakai Koutur"
base_domain="couturepro.app"
api_domain="api.massakai-koutur.couturepro.app"
app_domain="app.massakai-koutur.couturepro.app"
api_port="3005"
dir="$SHOPS_DIR/$slug"
...
if [ -f "/tmp/massakai_koutur_logo.jpg" ]; then
  echo "Attaching logo and luxury gold theme..."
  docker cp /tmp/massakai_koutur_logo.jpg massakai-koutur-api-1:/app/uploads/massakai_koutur_logo.jpg
  docker compose -f "$dir/docker-compose.yml" exec -T db psql -U couture -d couture_mali -c "UPDATE settings SET value = '\"/uploads/massakai_koutur_logo.jpg\"'::jsonb WHERE key = 'logo_url';"
...
certbot --nginx -d "$api_domain" -d "$app_domain" --non-interactive --agree-tos --register-unsafely-without-email || echo "Certbot warning (retry later if DNS still propagating)"

echo "[7/7] Adding daily backup cron..."
cron_line="32 3 * * * cd $dir && ./scripts/backup.sh >> /var/log/couture-backup.log 2>&1"
