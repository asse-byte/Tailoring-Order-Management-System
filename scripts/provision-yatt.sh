...
slug="yatt-services"
shop_name="YATT-SERVICES"
base_domain="couturepro.app"
api_domain="api.yatt-services.couturepro.app"
app_domain="app.yatt-services.couturepro.app"
api_port="3003"
dir="$SHOPS_DIR/$slug"
...
certbot --nginx -d "$api_domain" -d "$app_domain" --non-interactive --agree-tos --register-unsafely-without-email || echo "Certbot warning (check later)"

echo "[7/7] Adding daily backup cron..."
cron_line="21 3 * * * cd $dir && ./scripts/backup.sh >> /var/log/couture-backup.log 2>&1"
