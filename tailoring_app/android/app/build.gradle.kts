...
// Release signing: filled from android/key.properties when it exists (see
// docs/DEPLOYMENT.md). Absent on dev machines → we fall back to debug signing
// so `flutter run` keeps working without a keystore.
...
android {
    ...
    buildTypes {
        release {
            ...
            // to debug signing so dev builds still work without a keystore.
